...
#### To find the Fixed Points analytically
 ...
 Let us take the logistic model $x_{n} = f(x_{n-1})= 2x_{n-1}(1 - x_{n-1})$. Fixed points are those points where $2x_{n-1}^{*}(1 - x_{n-1}^{*}) = x_{n-1}^{*}$.

On simplifying,

$x_{n-1}^{*} - 2x_{n-1}^{*2} = 0$.

$x_{n-1}^{*} = 0 , x_{n-1}^{*} = \frac{1}{2}$ are the fixed points.
...
<figure>
![](../images/stability_fixed_pints.PNG)
...
<figure>
![](../images/2_return_map_logistic.PNG)
