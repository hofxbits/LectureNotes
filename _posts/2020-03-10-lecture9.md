...
Each time a Code is designed, the **Hamming Distance**($d$) between the codewords should be maximised to design an optimal error-correcting Code.    
...
Hence, the **Noisy Channel Coding theorem** states that if $\frac{H(\textbf{X})}{T_s} < \frac{\textbf{C}}{T_c}$, then, $\exists$ a Code $\mathbb C$ such that the probability of error $p_e < \epsilon$.   
